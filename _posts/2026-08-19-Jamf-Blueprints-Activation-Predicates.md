...
Now though, Apple have deprecated that configuration profile, and it's been confirmed it's [*removed* in macOS 27.0](https://github.com/apple/device-management/blob/seed_OS_27_0/mdm/profiles/com.apple.SoftwareUpdate.yaml#L12){:target="_blank"}
...
Doing it this way will mean you have two `Software Update Settings` entries under Device Declarations. One will be for the core settings, and the second will be for for the beta controls.
...
The beta controls declaration will show that it's denying Beta access, but it will also show it's setting other keys:
...
This isn't a *huge* issue at the moment, as most restrictive wins, but for ease of support it might be easier to only see the keys you're wanting to control.
...
This is because the snips and detail so far have only included examples where the device is *not a member* of the static group I included in my activation condition.
...
The above is just *one* example of what's possible leveraging activation predicates. There's lots of possibilities, and lots of power in the capability we're given here.<br>
